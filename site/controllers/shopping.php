...
class EnmasseControllerShopping extends JController
{
	...
	function addToCart()
	{
		...
		$deal->types = JModel::getInstance('dealType','enmasseModel')->getDealTypeListByIdDeal($dealId);
		...
		if(($deal->tier_pricing)==0)
		{
			...
			{
				$deal->type = JModel::getInstance('dealType','enmasseModel')->getById($typeId);
			...
			else
			{
				...
				if (!empty($deal->types))
				{				
					$deal->type = JModel::getInstance('dealType','enmasseModel')->getById($deal->types[0]->id);
					$deal->price = $deal->type->price;
				}
			}
		}
		...
		{		
			$deal->type = NULL;	
			$deal->price = EnmasseHelper::getCurrentPrice(unserialize($deal->price_step),$deal->price,$deal->cur_sold_qty);	
		}
		//*************************************************************************************
		// check Deal
		...
		else
		{
			...
			$max_buy_qty = $deal->max_buy_qty;
                        if($max_buy_qty == '-1' || ($max_buy_qty >=0 && $bought_qty < $max_buy_qty)){
                           ...
                           $msg = JText::_("User Bouht Enough Max Quantity. ").$item->name;
                            JFactory::getApplication()->redirect("index.php?option=com_enmasse&controller=shopping&task=checkout",$msg);
                        }  else {
                            $msg = JText::_("User Bought ITEM GREATER THAN MAX. ").$item->name;
...
//		$dealAttribute = JRequest::getVar('dealAttribute');
//		list($id,$origin)=explode("|", $dealAttribute);
//		if($origin)
//		{
//			$deal = JModel::getInstance('deal','enmasseModel')->getById($itemId);
//			if(($deal->tier_pricing)==1)
//			{
//				$newPrice = EnmasseHelper::getCurrentPriceStep(unserialize($deal->price_step),$deal->price,$deal->cur_sold_qty);
//				$calculatorPrice = EnmasseHelper::getCalculatorPrice(unserialize($deal->price_step),$newPrice,$deal->cur_sold_qty,$value);
//				
//				$newPrice = $calculatorPrice['newPrice'];	
//			}
//			else
//			{				
//				$newPrice = $deal->price;
...
//		else
//		{
//			$type = JModel::getInstance('dealType','enmasseModel')->getById($id);
//			$newPrice = $type->price;
//		}
//		$types = JModel::getInstance('dealType','enmasseModel')->getDealTypeListByIdDeal($itemId);
//		if(empty($value))
//		{
//			//$value = 1;
//			EnmasseControllerShopping::emptyCart();
//			$msg = JText::_( "EMPTY_CART");
//			$link = JRoute::_("index.php?option=com_enmasse&controller=deal&task=listing", false);
//			JFactory::getApplication()->redirect($link, $msg);
//				
...
//		$cart = unserialize(JFactory::getSession()->get('cart'));
//		CartHelper::checkCart($cart);
//		$cart->changeItem($itemId, $value,$newPrice,$types,$type);
//		JFactory::getSession()->set('cart', serialize($cart));
//
        ...
        function changeItem()
	{
		...
		CartHelper::checkCart($cart);
                ...
                foreach($oItems as $oCartItem){
                    ...
                    if($maxQtyOfDeal >= 0 && ($new_qty + $oCartItem->getCount()) > $maxQtyOfDeal)
                    {				
