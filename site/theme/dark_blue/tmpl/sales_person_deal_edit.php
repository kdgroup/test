	...
	<table class="admintable">
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="adminlist" id="dealTypeTbl">
				...
				<?php
					...
					foreach($this->dealTypeList as $dealType)
					{
						...
						<?php echo($dealType->name)?><input type="hidden" name="txtTypeName[]" value="<?php echo($dealType->name)?>">
												</td>
						...
						<td><?php echo($dealType->code); ?> <input type="hidden" name="txtTypeCode[]" value="<?php echo($dealType->code); ?>"></td>
						<td>
				...
				<tfoot>
					<tr>
						<td><input class="txtTypeName" type="text" name="txtTypeNameAdd" id="txtTypeName-1" title="Enter type name" value="" /></td>
		...
		<tr>
			...
			<td>
				...
				size="50" value="<?php echo $row->pic_dir;?>" />
...
function addDealTypes(){
	...
	if(dealTypeTbl.length > 0){
		...
		{
			alert("Price can't be greater than orginal price");
			return false;
		}
