<?php
/**
 * This file defines the Dummy class
 * 
 * (c) Some Comp
 *
...
JFactory::getDocument()->addScript("components/com_enmasse/theme/js/dealtype.js");
...
<div class="deal">	
    <div class="main_deal">
        <div id="rss"><a href="<?php echo JRoute::_('index.php?option=com_enmasse&controller=rss&task=deal&id=' . $deal->id); ?>"><img src="components/com_enmasse/theme/<?php echo $theme ?>/images/rss.gif" alt="Deal RSS Feed" title="RSS Feed"/></a></div>		
...
<?php if ($deal->types) { ?><p class='fromprc'><?php echo JText::_('FROM') ?></p><?php } ?>
...
<?php elseif (strtotime($deal->start_at) > strtotime(DatetimeWrapper::getDatetimeOfNow())): ?>
                    ...
                    <?php else : ?>
                        ...
                        ?>index.php?option=com_enmasse&controller=shopping&task=addToCart&dealId=<?php echo $deal->id . '&slug_name=' . $deal->slug_name; ?>&referralid=<?php echo JRequest::getVar('referralid'); ?>&typeId=none<?php } ?>" class="buy"><?php echo JText::_('Buy') ?></a>
                        <?php
                        if ($deal->types) {
                            ?>
                            <a href="#" class="showBuy"><?php echo JText::_('Buy') ?></a>
                           <?php } ?>
                       <?php endif; ?> 				
                </div>
            </div>
            ...
            <div id="for_a_friend">
                <a class="icon_gift <?php if ($deal->types) echo 'showBuy'; ?> " href="<?php if (!$deal->types) { ?>index.php?option=com_enmasse&controller=shopping&task=addToCart&dealId=<?php echo $deal->id . "&buy4friend=1&slug_name=" . $deal->slug_name;
                    }
                    else echo 'javascript:void();' ?>"><?php echo JText::_('BUY_IT_FOR_FRIEND') ?></a>
            </div>
...
<?php if (isset($this->upcoming)): ?>
                ...
                <?php else: ?>
    <?php
    $bDealOn = (($deal->min_needed_qty - $deal->cur_sold_qty) <= 0);
    if ($bDealOn): ?>
                    ...
                    <?php if (!$bDealOn || ($deal->max_coupon_qty != "-1" && $deal->cur_sold_qty != $deal->max_coupon_qty)): ?>
                        ...
                        <div class="point">
                            ...
                            <div class="centerpoint">
                                ...
                                <div style="width: <?php echo $bDealOn ? 159 * ($deal->cur_sold_qty / $deal->max_coupon_qty)-6 : 159 * ($deal->cur_sold_qty / $deal->min_needed_qty) -1; ?>px;" class="centervote"></div>
...
<?php if ($deal->pic_dir): ?>
    <?php if (count($imageUrlArr) == 1): ?>
                        <img width="<?php echo $image_width; ?>" height="<?php echo $image_height; ?>" src="<?php echo str_replace("\\", "/", $imageUrlArr[0]); ?>" alt="<?php echo $deal->name; ?>" />
                    <?php else : ?>
                        <script type="text/javascript">
                            var itv = 0;
                            var time_duration = 4000;
                            var stop = false;
                            var imgIndex = 0;
                            var isLoaded = false;
                            function loadBlinds() {
                                    if(isLoaded) return;
                                    jQuery(jQuery('.change_link')[0]).addClass('active');
                                    jQuery('.slideshow').blinds();
                                    itv = window.setTimeout("beginSlide()", time_duration);
                                    isLoaded = true;
                            }
                            function beginSlide(){
                                    if(!stop) {
                                            itv = clearTimeout(itv);
                                            jQuery('.slideshow').blinds_change(imgIndex);
                                            imgIndex = (imgIndex + 1)% jQuery('.slideshow ul li').length;
                                            jQuery('.change_link').removeClass('active');
                                            jQuery(jQuery('.change_link')[imgIndex]).addClass('active');
                                            itv = window.setTimeout("beginSlide()", time_duration);
                                    }
                            }
                            ...
                            jQuery(document).ready(function () {
                                    jQuery(jQuery('.slideshow img')[0]).load(function() {
                                            loadBlinds();
                                    });
                                    setTimeout(function(){
                                            loadBlinds();
        ...
        <?php for ($i = 0; $i < count($imageUrlArr); $i++): ?>
                                    <li><img src="<?php echo str_replace("\\", "/", $imageUrlArr[$i]) ?>" height="<?php echo $image_height ?>" width="<?php echo $image_width ?>" alt="<?php echo $deal->name; ?>" /></li>
                            <?php endfor; ?>
                            </ul>
                        </div>
                        <div>
    ...
    <?php endif; ?>
        ...
        if ($deal->types) {
            ?>
            <div class="popupContBuy hidden" id='popupContBuynormal' style="left:215px; top:159px;">
                                ...
                                <tbody>
                                    ...
                                    foreach ($deal->types as $type) {
                                        ...
                                        <tr class='<?php echo($class_tr); ?>'>
                                            ...
                                            <td class="last">
                                                <p><a href="index.php?option=com_enmasse&controller=shopping&task=addToCart&dealId=<?php echo $deal->id . '&slug_name=' . $deal->slug_name; ?>&referralid=<?php echo JRequest::getVar('referralid'); ?>&typeId=<?php echo($type->id); ?>" class="button"><?php echo(EnmasseHelper::displayCurrency($type->price)); ?></a></p>
                                            </td>
                                        </tr>
                                        <?php
                                    }
            ...
            <div class="popupContBuy  hidden" id='popupContBuyFriend' style="left:215px; top:159px;">
                                ...
                                <tbody>
                                    ...
                                    foreach ($deal->types as $type) {
                                        ...
                                        <tr class='<?php echo($class_tr); ?>'>
                                            ...
                                            <td class="last">
                                                <p><a href="index.php?option=com_enmasse&controller=shopping&task=addToCart&dealId=<?php echo $deal->id . '&slug_name=' . $deal->slug_name; ?>&referralid=<?php echo JRequest::getVar('referralid'); ?>&typeId=<?php echo($type->id); ?>&buy4friend=1" class="button"><?php echo(EnmasseHelper::displayCurrency($type->price)); ?></a></p>
                                            </td>
                                        </tr>
        <?php
    }
...
<?php echo $deal->description ?>
            ...
            <div class="comment_button">
                <input class='button' type="button" value="<?php echo JText::_('COMMENT_BUTTON'); ?>" onclick="window.location = '<?php echo $sCommentLink; ?>'"></input>
            </div>
        ...
        <div id="rightcol" class="deal_cont_main_right">
            <?php if ($merchant): ?>
				...
				<div>
                                    <?php if($logoImages){ ?>
					<img src="<?php echo  $logoImages;?>" style="max-width: 200px"/>
                                    <?php } ?>
				</div>
        ...
        ?>
                    ...
                    <?php $count = 1;
                    foreach ($merchant->branches as $branch): ?>
                        <div id="branch<?php echo $count ?>" style="<?php echo $count === 1 ? 'display:block' : 'display:none'; ?>">
