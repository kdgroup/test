...
<div class="em_dealMain checkOut">
    <h3>Check Out</h3>
</div>
<div class="deal">   
	<div class="main_deal">
		...
		<div class="h13"></div>
		<form action='index.php' id="adminForm" name="adminForm"  class="form-validate" method="post" onSubmit="return myValidate(this);">
...
//							}
		        ...
		        <div class="line">
			        <?php if (isset($this->termArticleId) && $this->termArticleId!=0):?>
				        <input style="margin-top:-5px" type="checkbox" name="terms" id="terms" class="required" <?php if(isset($this->arData['terms'])) echo "checked=\"checked\""?>>
				    	<label for="terms" style="display:inline;">
				    		<?php echo JText::_('SHOP_CARD_CHECK_OUT_TERM_CONDITION');?>
				        	<a style="float:none" href="<?php echo JRoute::_('index.php?option=com_enmasse&controller=term&id='.$this->termArticleId).'&tmpl=component'; ?>"
								onclick="window.open('<?php echo JRoute::_('index.php?option=com_enmasse&controller=term&id='.$this->termArticleId).'&tmpl=component'; ?>','name','height=600,width=400,toolbar=no,directories=no,status=no,menubar=no,scrollbars=1,resizable=no'); return false;">
		    ...
		    <div class="bottom">
				<input type="submit" class="button" value="<?php echo JText::_('PROCESS_CHECK_OUT_BUTTON');?>" ></input>
		    </div>
		</div>
		<?php }
		 else
