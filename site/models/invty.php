...
class EnmasseModelInvty extends JModel
{
	...
	function generateForOrderItem($pdtId, $orderItemId, $qty, $status)
	{
		...
		for ( $j = 0; $j < $qty; $j++)
		{
			$random = EnmasseHelper::makeAnSecurityCode();
			$name =$pdtId;
			$name.="-";
			$name.=$orderItemId;
			$name.="-";
			...
			$query = "INSERT INTO #__enmasse_invty (name, order_item_id, pdt_id, status,created_at,`security_code`) VALUES ('".$name."',".$orderItemId.",'".$pdtId."','".$status."','".$created_at."','{$random}')";
			$db->setQuery( $query );
			$db->query();
	...
	function generateCouponFreeStatus($pdtId,$couponName,$status)
	{
		...
		$db = JFactory::getDBO();
		$created_at = DatetimeWrapper::getDatetimeOfNow();
		$query = "INSERT INTO #__enmasse_invty (name, pdt_id, status,created_at,`security_code`) VALUES ('".$couponName."','".$pdtId."','".$status."','".$created_at."', '{$random}')";
		$db->setQuery( $query );
		$db->query();
	    if ($this->_db->getErrorNum()) {
				JError::raiseError( 500, $this->_db->stderr() );
