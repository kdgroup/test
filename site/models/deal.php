...
class EnmasseModelDeal extends JModel
{
	...
	function searchStartedPublishedDeal($keyword=null, $categoryId=null, $locationId=null, $sortBy=null,$limit_row=NULL)
    {
        ...
        if ($limit_row)
        {
            $limit_row = intval($limit_row);
            $query .= "  LIMIT 0, {$limit_row}";
        }
    //echo $query;
        $db->setQuery( $query );
...
// end update
	...
	function ws_todayDealList($location_id = null ,$limit_row = 10)
	{
		$location_condition = "";
        if($location_id)
            $location_condition = " AND id IN (SELECT loc.deal_id FROM #__enmasse_deal_location loc WHERE loc.location_id = $location_id ) ";
	...
	function ws_startedPublishedDealList($location_id = null ,$limit_row = 10)
    {
        $db = JFactory::getDBO();
        // generate the query
        $query = "    SELECT 
        ...
        if($location_id != null)
            $query .=" AND d.id IN (SELECT loc.deal_id FROM #__enmasse_deal_location loc WHERE loc.location_id = $location_id ) ";
         
		$query .= "    ORDER BY d.start_at DESC";
	...
	function ws_upcomingDealList($location_id = null ,$limit_row = 10)
	{
		$db = JFactory::getDBO();
		$query = "SELECT d.* 
					FROM 
						#__enmasse_deal d 
					WHERE
				  		status NOT LIKE 'Pending' AND published = '1' ";
		
        if($location_id)
            $query .=" AND d.id IN (SELECT loc.deal_id FROM #__enmasse_deal_location loc WHERE loc.location_id = $location_id ) ";
         
		$query .= " AND d.start_at > '".DatetimeWrapper::getDatetimeOfNow()."'";
    ...
    function ws_expiredPublishedDealList($location_id = null ,$limit_row = 10)
	{
		$db = JFactory::getDBO();
		...
		return $rows;                
          ...
          function searchStartedPublishedDealByListCat($keyword=null, $categoryId=null, $locationId=null, $sortBy=null,$limit_row=NULL)
    {
        ...
        if($keyword != null)
        {
            $keyword = EnmasseHelper::escapeSqlLikeSpecialChar($db->escape($keyword));
            $query .= "    AND d.name like '%$keyword%'";
        }
        if($categoryId)
            $query .=" AND d.id IN (SELECT cat.deal_id FROM #__enmasse_deal_category cat WHERE cat.category_id in (".$categoryId.") ) ";
