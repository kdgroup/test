...
class EnmasseControllerDeal extends JControllerLegacy
{  
	...
	function save()
	{
		...
		$price_step_arr = $data['price_step'];
		
                $data['price'] = (float)$data['price'];
		if($data['tier_pricing']==1)
		{
			if($price_step_arr){
				$min_quantity_arr = $data['min_quantity_step'];
				
				$price_step = array();
                                $price_step[$data['price']] = 0;
				foreach ($min_quantity_arr as $k=>$v)
				{
					if(($v != '' && $k != '' || $v != '' && $k == '0') && $price_step_arr[$k] != '' )
					{
						$price_step[$price_step_arr[$k]] = $v;
					}
				}
				$data['price_step'] 		= serialize($price_step);
//				$checkchracter = EnmasseHelper::checkSpecialCharacter($data['name']);
			}
		}
                if(!$data['pic_dir'])
		{			
			$data['pic_dir'] = 'a%3A1%3A%7Bi%3A0%3Bs%3A60%3A%22components%5Ccom_enmasse%5Cupload%5Ci20130515043120dealdefault.jpg%22%3B%7D';
		}
		$checkchracter = EnmasseHelper::checkSpecialCharacter($data['name']);
		if($checkchracter)
		{
