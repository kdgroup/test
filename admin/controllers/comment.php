...
class EnmasseControllerComment extends JController
{
    ...
    function publish()
    {
        ...
        $bResult = JModel::getInstance('Comment', 'EnmasseModel')->changeCommentStatus((int) $sCommentIds, 2);
        if($bResult)
        {
    ...
    function unpublish()
    {
        ...
        JArrayHelper::toInteger($aCid);
        $sCommentIds = implode(',', $aCid);
        $bResult = JModel::getInstance('Comment', 'EnmasseModel')->changeCommentStatus((int) $sCommentIds, 1);
        if($bResult)
        {
