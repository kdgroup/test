...
class EnmasseControllerCategory extends JController
{
	...
	function save()
	{
		...
		if(strlen($data['name']) > 50 || strlen($data['name']) < 1)
		{
	...
	function remove()
	{
		...
		{
			$dealList = JModel::getInstance("dealCategory","enmasseModel")->getDealByCategoryId((int) $cids[$count]);
                        //search all sub category need remove
                        $subCategoryLs = JModel::getInstance("category","enmasseModel")->getSubCategory((int) $cids[$count]);
                        $haveDeal = FALSE ;
                        //check whether sub category have contain any deal?
                        foreach ($subCategoryLs as $subCategory) {
                           $tmp = JModel::getInstance("dealCategory","enmasseModel")->getDealByCategoryId((int) $subCategory);
                           if($tmp){
                               $haveDeal = true;
                                break;
                           }
                        }
                        // if category have contain deal or sub of category have contain,so have to remove out
                        //list category need remove
			if(count($dealList)!=0 || $haveDeal)
			{
				...
				unset($cids[$count]);				
                            ...
                            //if both parent and sub are not have deal --> delete sub them
                            $model->deleteList($subCategoryLs);
                        }
