...
class EnmasseControllerSalesPerson extends JController
{
	...
	function save()
	{
		...
		$email = $data['email'];
		//$checkEmal = preg_match($email,'\b[A-Z0-9._%-]+@[A-Z0-9.-]+\.[A-Z]{2,4}\b');
		
		$checkEmal  = preg_match("/^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,3})$/", $email);
		
		
		if (!$checkEmal)
		{
			$link='index.php?option=com_enmasse&controller=salesPerson&task=edit&cid[]='.$data['id'];
			$msg = JText::_('PLEASE_ENTER_VALID_EMAIL');
			JFactory::getApplication()->redirect($link, $msg, 'error');
		}
		
		$checkPhoneNum = preg_match("/^[0-9]+$/",  $data['phone']);
		if(!$checkPhoneNum)
		{
			$link='index.php?option=com_enmasse&controller=salesPerson&task=edit&cid[]='.$data['id'];
			$msg = JText::_('PLEASE_ENTER_VALID_PHONE');
			JFactory::getApplication()->redirect($link, $msg, 'error');
		}
