...
class EnmasseModelCategory extends JModel
{
	...
	function listAllPublished()
	{
        ...
        {
                $db 	= JFactory::getDBO();
		$query 	= 'SELECT * FROM #__enmasse_category WHERE parent_id = '.$db->quote('').' OR parent_id is null';
		$db->setQuery($query);
	...
	function getCategoryByName($name)
	{
        ...
        function getSubCategory($idCate){
            $oDb = JFactory::getDBO();
            $oQuery = $oDb->getQuery(true);
            $oQuery->select('id');
            $oQuery->from('#__enmasse_category');
            $oQuery->where('parent_id = '.$idCate);
            $oDb->setQuery($oQuery);
            $subCateIdArr = $oDb->loadResultArray();
