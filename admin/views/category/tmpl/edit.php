...
<table class="admintable" style="width: 100%">
	<tr>
		...
		<td><input class="text_area" type="text" name="name" id="name"
         ...
         <tr>
            <td align="right"><?php echo JTEXT::_('TOOLTIP_CATEGORY_PARENT_NAME'); ?></td>
            <td>
            <?php
                $options[] = JHtml::_('select.option', '', JText::_('-- Select --'));
                foreach ($this->parent_category as $category) {
                        if($row->id!=$category->id){
                            $options[] = JHtml::_('select.option', $category->id, $category->name);
                        }
                }
                echo JHtml::_('select.genericlist', $options, 'parent_id', '', 'value', 'text', $row->parent_id);
            ?>
            </td>
        </tr>
	<tr>
