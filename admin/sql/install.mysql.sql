...
CREATE TABLE IF NOT EXISTS `#__enmasse_deal` (
  ...
  `tier_pricing` tinyint(3) NOT NULL DEFAULT '0',
  `price_step` mediumtext COLLATE utf8_unicode_ci,
  ...
  `commission_percent` tinyint(1) DEFAULT '0',
  PRIMARY KEY (`id`),
  UNIQUE KEY `name` (`name`),
  UNIQUE KEY `slug_name` (`slug_name`),
...
CREATE TABLE IF NOT EXISTS `#__enmasse_order_item` (
  ...
  `created_at` datetime NOT NULL,
  `updated_at` datetime NOT NULL,
  `attr_info` varchar(255) COLLATE utf8_unicode_ci DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `#__enmasse_setting` (
  ...
  `enable_security_code` TINYINT UNSIGNED NOT NULL DEFAULT 0,
  `enable_rtl` tinyint(3) DEFAULT NULL,
  PRIMARY KEY (`id`),
  FULLTEXT KEY `logo_url` (`logo_url`)
) ENGINE=MyISAM  DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci AUTO_INCREMENT=2 ;


INSERT IGNORE INTO `#__enmasse_setting` (`company_name`, `id`, `address1`, `address2`, `city`, `state`, `country`, `postal_code`, `tax`, `tax_number1`, `tax_number2`, `logo_url`, `contact_number`, `contact_fax`, `customer_support_email`, `default_currency`, `currency_prefix`, `currency_postfix`, `currency_decimal`, `currency_separator`, `currency_decimal_separator`, `image_height`, `image_width`, `article_id`, `subscription_class`, `theme`, `mobile_theme`, `coupon_bg_url`, `minute_release_invty`, `cash_minute_release_invty`, `created_at`, `updated_at`, `point_system_class`, `active_guest_buying`, `sale_group`, `merchant_group`, `sending_bill_auto`, `delivery_group`, `enable_security_code`, `enable_rtl`) VALUES
('Your company name', 1, 'Your company''s address', '', 'Singapore', 'Singapore', 'SG', '12345', '', '', '', '', '', '', 'support@yourcompany.com', 'USD', '$', '', 2, ',', '.', 300, 560, 1, 'acystarter', 'dark_blue', 'mobile', 'a%3A1%3A%7Bi%3A0%3Bs%3A51%3A%22components%5Ccom_enmasse%5Cupload%5C18040samplecoupon.png%22%3B%7D', 10, 2, '0000-00-00 00:00:00', '2012-01-06 14:28:23', 'no', 0, 9, 10, 0, 11, 0, 0);
